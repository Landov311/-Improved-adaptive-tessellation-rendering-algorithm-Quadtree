...
#ifndef QUADTREE_FBX_H
#define QUADTREE_FBX_H
//
// Created by lvera on 29/06/2025.
//
...
private:
    int max_depth;
    std::tuple<float, float, float, float> calculateUVBounds(const std::vector<Vertex>& vertices) {
    ...
    // Función privada para asignar vértices
    void assignVerticesToNodes(QuadtreeNode* node, const std::vector<Vertex>& vertices,
                             float minU, float maxU, float minV, float maxV) {
        auto normalizeUV = [&](float u, float v) {
        ...
        for (size_t i = 0; i < vertices.size(); ++i) {
            auto [normU, normV] = normalizeUV(vertices[i].texCoords.x, vertices[i].texCoords.y);
            if (u_bounds.first <= normU && normU <= u_bounds.second &&
                v_bounds.first <= normV && normV <= v_bounds.second) {
                vertex_indices.push_back(i);
                }
        }

        if (!vertex_indices.empty() && node->getDepth() < max_depth) {
            auto children = node->subdivide();
            for (auto* child : children) {
                assignVerticesToNodes(child, vertices, minU, maxU, minV, maxV);
            }
        }
    }
public:
    ...
    std::unique_ptr<QuadtreeNode> buildFromFBX(const std::vector<Vertex>& vertices) {
        if (vertices.empty()) {
